...
pub const PlusTool = struct {
    ...
    pub fn use(self: *PlusTool, input_text: []const u8) Result {
        ...
        const start_idx = std.mem.indexOf(u8, input_text, action_str) orelse return Result{ .error = "SyntaxError: invalid syntax." };
        
        // Extract the part after the action
        const after_action = input_text[start_idx + action_str.len..];
        
        // Find the closing ```
        const end_idx = std.mem.indexOf(u8, after_action, "```") orelse return Result{ .error = "SyntaxError: invalid syntax." };
        ...
        const num1_str = iter.next() orelse return Result{ .error = "SyntaxError: invalid syntax." };
        const num2_str = iter.next() orelse return Result{ .error = "SyntaxError: invalid syntax." };
        
        // Check if there are extra numbers (more than 2)
        if (iter.next()) |_| {
            return Result{ .error = "SyntaxError: invalid syntax." };
        }
        
        // Parse the numbers
        const num1 = std.fmt.parseInt(i64, num1_str, 10) catch return Result{ .error = "ValueError: invalid value." };
        const num2 = std.fmt.parseInt(i64, num2_str, 10) catch return Result{ .error = "ValueError: invalid value." };
